...
async function runAudioSequence(burnActions) {
    ...
    const shufflerName = gameState.players[shIdx]?.name;

    if (shufflerName) {
        await speak(`${shufflerName} punten kocok kartuna`);
    }
    ...
    // AI Comment
    const comment =
        AI_COMMENTS[Math.floor(Math.random() * AI_COMMENTS.length)];
    ...
    if (
        sorted.length >= 2 &&
        (sorted[0].score - sorted[1].score) <= 50
    ) {
        await speakGameEvent('close');
    }

    const minusPlayer = gameState.players.find(
        p => p.score <= -100
    );
