...
// ============================================================
// ============================================================
// BURN SEQUENCE SYSTEM
// ============================================================
// Struktur folder di server (taruh sejajar dengan index.html):
//   attack/images/dragon_card.jpg
//   attack/images/tiger_card.jpg
//   attack/images/eagle_card.jpg
//   attack/images/cobra_card.jpg
//   attack/sound animal/dragon.mp3
//   attack/sound animal/tiger.mp3
//   attack/sound animal/eagle.mp3
//   attack/sound animal/cobra.mp3
//   attack/sound efek/dragon_SFX.mp3
//   attack/sound efek/tiger_SFX.mp3
//   attack/sound efek/eagle_SFX.mp3
//   attack/sound efek/cobra_SFX.mp3
//   attack/dragon/dragon_1.png … dragon_122.png
//   attack/tiger/tiger_1.png … tiger_122.png
//   attack/eagle/eagle_1.png … eagle_122.png
//   attack/cobra/cobra_1.png … cobra_122.png

const ATTACK_FRAME_COUNT = 122;
const ATTACK_ANIMAL_SOUNDS = ELEMENT_NAMES.map(n => `attack/sound animal/${n}.mp3`);
const ATTACK_SFX_SOUNDS    = ELEMENT_NAMES.map(n => `attack/sound efek/${n}_SFX.mp3`);
const ATTACK_CARD_IMGS     = ELEMENT_NAMES.map(n => `attack/images/${n}_card.jpg`);
const ATTACK_FRAME_DIRS    = ELEMENT_NAMES.map(n => `attack/${n}/${n}_`);
...
function handleConfirmBurn() {
  ...
  // Sembunyikan burn panel
  const bp = document.getElementById('burn-panel');
  if (bp) bp.style.display = 'none';

  const runAll = async () => {
    for (const ba of burnActionsForAudio) {
      const victimScoreBefore = gameState.players[ba.victimIdx]?.score || 0;
      await playBurnSequence(ba.attackerIdx, ba.victimIdx, victimScoreBefore);
    }
...
// Orkestrator satu sekuens burn
function playBurnSequence(attackerIdx, victimIdx, victimScoreBefore) {
  return new Promise(resolve => {
    showAttackerCard(attackerIdx, () => {
      playAnimalSound(attackerIdx, () => {
        hideAttackerCard(() => {
          // Animasi PNG + SFX + getar border dulu
          // Teks -skor muncul SETELAH animasi PNG selesai
          playFrameAnimation(attackerIdx, victimIdx, () => {
            if (victimScoreBefore !== 0) {
              showScoreMinusEffect(victimIdx, victimScoreBefore);
            }
            // Tunggu teks -skor selesai melayang baru resolve
...
// ── Tahap 1: Kartu Yu-Gi-Oh muncul di tengah ─────────────────
let _burnCardEl = null;
let _burnOverlayEl = null;

function showAttackerCard(attackerIdx, onDone) {
  const overlay = document.createElement('div');
  overlay.className = 'burn-overlay-bg';
  document.body.appendChild(overlay);
  _burnOverlayEl = overlay;

  const card = document.createElement('div');
  card.className = 'burn-yugioh-card';
  const img = document.createElement('img');
  img.src = ATTACK_CARD_IMGS[attackerIdx];
  ...
  requestAnimationFrame(() => {
    requestAnimationFrame(() => {
      card.classList.add('burn-card-enter');
    });
  });
...
// ── Tahap 2: Suara hewan ──────────────────────────────────────
function playAnimalSound(attackerIdx, onDone) {
  const audio = new Audio(ATTACK_ANIMAL_SOUNDS[attackerIdx]);
  audio.onended = () => onDone();
  audio.onerror = () => onDone();
  audio.play().catch(() => onDone());
}

// ── Tahap 3: Kartu menghilang ─────────────────────────────────
function hideAttackerCard(onDone) {
  if (!_burnCardEl) { onDone(); return; }
  _burnCardEl.classList.remove('burn-card-enter');
  _burnCardEl.classList.add('burn-card-exit');
  if (_burnOverlayEl) _burnOverlayEl.classList.add('burn-overlay-fade');
  setTimeout(() => {
    if (_burnCardEl)    { _burnCardEl.remove();    _burnCardEl = null; }
    if (_burnOverlayEl) { _burnOverlayEl.remove(); _burnOverlayEl = null; }
    onDone();
  }, 500);
}

// ── Efek teks -skor melayang ke atas ─────────────────────────
function showScoreMinusEffect(victimIdx, score) {
  const card = document.getElementById(`card-${victimIdx}`);
  if (!card) return;
  const rect = card.getBoundingClientRect();
  const el = document.createElement('div');
  el.className = 'burn-score-minus';
  el.textContent = `-${score}`;
  el.style.cssText = `left:${rect.left + rect.width/2}px;top:${rect.top + rect.height/2}px;`;
  document.body.appendChild(el);
  setTimeout(() => el.remove(), 1400);
}

// ── Tahap 4: Animasi 122 PNG (lazy load + cleanup) ───────────
function playFrameAnimation(attackerIdx, victimIdx, onDone) {
  const victimCard = document.getElementById(`card-${victimIdx}`);
  if (!victimCard) { onDone(); return; }

  const rect = victimCard.getBoundingClientRect();
  const centerX = rect.left + rect.width  / 2;
  const centerY = rect.top  + rect.height / 2;

  // SFX + border korban bergetar saat animasi dimulai
  const sfx = new Audio(ATTACK_SFX_SOUNDS[attackerIdx]);
  sfx.play().catch(() => {});
  ...
  canvas.width  = 640;
  canvas.height = 360;
  canvas.className = 'burn-frame-canvas';
  canvas.style.cssText = `left:${centerX - 320}px;top:${centerY - 180}px;`;
  document.body.appendChild(canvas);
  const ctx = canvas.getContext('2d');

  const prefix = ATTACK_FRAME_DIRS[attackerIdx];
  let currentFrame = 1;
  let img = new Image();
  ...
  function loadNext() {
    ...
    img.src = `${prefix}${currentFrame}.png`;
  }
